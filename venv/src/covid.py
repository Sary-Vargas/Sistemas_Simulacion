...
class GameOfLifeApp:
    def __init__(self, root, main_window=None):
        ...
        self.root.title("COVID-19 Simulación 2D (Expansión dinámica)")
        ...
        self.run_btn = tk.Button(btn_frame, text="Ejecutar", command=self._toggle_run,
                                font=("Arial", 10), width=10, bg="#E91E63", fg="white")
    ...
    def _draw_grid(self):
        ...
        for r in range(self.sim.rows):
            for c in range(self.sim.cols):
                ...
                x2 = x1 + CELL_SIZE
                y2 = y1 + CELL_SIZE
                fill = "red" if self.sim.grid[r, c] else "white"  # Rojo para simular COVID
    ...
    def _toggle_run(self):
        ...
        self.run_btn.config(text="Parar" if self.running else "Ejecutar",
                           bg="#F44336" if self.running else "#E91E63")
        if self.running:
            self._run_loop()
    ...
    def _reset(self):
        self.running = False
        self.run_btn.config(text="Ejecutar", bg="#E91E63")
        self.sim = GameOfLife2D()
        self._draw_grid()
